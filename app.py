...
def generate():
    ...
    for i, f in enumerate(uploaded):
        ...
        try:
            ...
            app.logger.exception("Error parsing uploaded file %s", f.filename)
            return "Error parsing uploaded file: invalid or corrupted file", 400
    ...
    if vcf9_field in ('1', 'true', 'on', 'yes'):
        try:
            ...
            app.logger.warning('VCF9 check failed', exc_info=True)

    try:
        excalidraw_json = generate_excalidraw(sites, vcf9_enabled=vcf9_enabled)
    except (ValueError, KeyError, TypeError) as e:
        app.logger.exception("Diagram generation error")
        return "Diagram generation error: please check your input files", 500
...
def _parse_sites_for_vcf9(uploaded, names):
    ...
    for i, f in enumerate(uploaded):
        ...
        try:
            ...
            app.logger.exception("Error parsing uploaded file %s", f.filename)
            return None, "Error parsing uploaded file: invalid or corrupted file"
    if not sites:
